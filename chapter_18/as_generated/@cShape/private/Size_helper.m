...
%SIZE_HELPER for class cShape, Replace with a short note that will appear on
...
switch which
    ...
    case 'set'  % MUTATOR
        ...
        %     [this.Size] = deal(varargin{:});
        % else  % deeper indexing requested, use subsasgn to do it
        %     Size = [this.Size];  % Modify the assignment
        %     Size = subsasgn(Size, index, varargin{:});
        %     [this.Size] = Size;
        % end
        % /\  /\  /\  /\
    otherwise

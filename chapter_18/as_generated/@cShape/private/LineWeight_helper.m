...
%LINEWEIGHT_HELPER for class cShape, Replace with a short note that will appear
%                  on the H1 line along with each function description
%
...
switch which
    ...
    case 'set'  % MUTATOR
        ...
        %     [this.LineWeight] = deal(varargin{:});
        % else  % deeper indexing requested, use subsasgn to do it
        %     LineWeight = [this.LineWeight];  % Modify the assignment
        %     LineWeight = subsasgn(LineWeight, index, varargin{:});

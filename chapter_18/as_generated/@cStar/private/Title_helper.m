...
%TITLE_HELPER for class cStar, Replace with a short note that will appear on
%             the H1 line along with each function description
%
%           Replace with something like UNCLASSIFIED
%
% function [do_sub_indexing, do_assignin, this, varargout] = Title_helper(which,
%      this, index, varargin)
%
...
switch which
    ...
    case 'set'  % MUTATOR
        ...
        %     [this.Title] = deal(varargin{:});
        % else  % deeper indexing requested, use subsasgn to do it
        %     Title = [this.Title];  % Modify the assignment
        %     Title = subsasgn(Title, index, varargin{:});
        %     [this.Title] = Title;
        % end
        % /\  /\  /\  /\
    otherwise

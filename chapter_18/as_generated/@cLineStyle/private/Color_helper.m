...
%COLOR_HELPER for class cLineStyle, Replace with a short note that will appear
...
switch which
    ...
    case 'set'  % MUTATOR
        ...
        %     [this.Color] = deal(varargin{:});
        % else  % deeper indexing requested, use subsasgn to do it
        %     Color = [this.Color];  % Modify the assignment
        %     Color = subsasgn(Color, index, varargin{:});
        %     [this.Color] = Color;
        % end
        % /\  /\  /\  /\
    otherwise

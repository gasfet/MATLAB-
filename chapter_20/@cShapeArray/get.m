...
switch index(1).subs
    otherwise
        found = false;  % now lets look in each object
        for k = 1:numel(this.mArray)
            try
                varargout{k} = get(this.mArray{k}, index, varargin{:});
                do_sub_indexing = false; % object must sub-index
                found = true;
            catch
                err = lasterror;
...
if ~found && called_by_name
    ...
    switch index(1).subs
        ...
        otherwise
            found = false;  % look in each object
            for k = 1:numel(this.mArray)
                try
                    varargout{k} = get(this.mArray{k}, index(1).subs, varargin{:});
                    do_sub_indexing = false; % object must sub-index
                    found = true;
                catch
                    err = lasterror;

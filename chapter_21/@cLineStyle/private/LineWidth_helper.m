...
switch which
    ...
    case 'set'  % MUTATOR
        ...
        static_this = static;
        for width = [varargin{:}]
            if length(static_this.mLineWidthCounter) < width
                static_this.mLineWidthCounter(width) = 1;
            else
                static_this.mLineWidthCounter(width) = ...
                    1 + static_this.mLineWidthCounter(width);
            end
        end
        static(static_this);
